...
func centerInBox(text, textColor, textStyle, boxWidth, borderColor)
	padding = floor((boxWidth - len(text)) / 2)
	rightPadding = boxWidth - padding - len(text)
	
	leftPart = colorText([:text = "│" + copy(" ", padding), :color = borderColor])
	centerPart = colorText([:text = text, :color = textColor, :style = textStyle])
	rightPart = colorText([:text = copy(" ", rightPadding) + "│", :color = borderColor])
	
	return leftPart + centerPart + rightPart

func colorBox(boxLine, color, style)
	return colorText([:text = boxLine, :color = color, :style = style])
